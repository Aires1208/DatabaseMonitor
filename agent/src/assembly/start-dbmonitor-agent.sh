...
APP_PACKAGE=`basename $RUNHOME/*.zip`
APP_NAME=`echo $APP_PACKAGE | sed -r 's/-((v|V)|[0-9])(.*).zip//g'`
APP_PORT=38205
...
CLASS_PATH="$RUNHOME/$APP_NAME:$RUNHOME/$APP_NAME/lib/*:$RUNHOME/$APP_NAME/dbmonitor-agent.jar"
...
{
	cd $RUNHOME
	unzip -o -q -d $APP_NAME $APP_PACKAGE
}
...
create_log()
{
	...
	echo  @JAVA_OPTS@ $JAVA_OPTS >> $LOGS_DIR/$LOG_FILE
  	echo  @CLASS_PATH@ $CLASS_PATH >> $LOGS_DIR/$LOG_FILE
	echo  @MAIN_CLASS@ $MAIN_CLASS >> $LOGS_DIR/$LOG_FILE
...
start_app()
{
	...
	pid=$(nohup $JAVA $JAVA_OPTS -classpath $CLASS_PATH $MAIN_CLASS server $RUNHOME/$APP_NAME/itm-agent.yml >> $LOGS_DIR/$LOG_FILE 2>&1 & echo $!)
